    ...
    <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">Logins, page views, and key actions — last 500 events</p>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={loadLogs}
            className="text-xs bg-slate-100 text-slate-600 px-3 py-1.5 rounded-md hover:bg-slate-200">Refresh</button>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0">
              <tr className="bg-slate-50 text-left">
                ...
                <th className="px-4 py-2 font-medium text-slate-500">Event</th>
                <th className="px-4 py-2 font-medium text-slate-500">Detail</th>
            ...
            <tbody>
              {filtered.map((l) => (
                <tr key={l.id} className="border-t border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="px-4 py-2">
                    <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${typeBadge[l.event_type] || "bg-slate-100 text-slate-600"}`}>
                      {l.event_type === "page_view" ? "page view" : l.event_type}
                    </span>
                  </td>
                  <td className="px-4 py-2 text-slate-600">{l.event_detail || "—"}</td>
                  <td className="px-4 py-2 text-slate-400 text-xs whitespace-nowrap" title={new Date(l.created_at).toLocaleString()}>
